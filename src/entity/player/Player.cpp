...
void Player::placeBomb(const std::vector<Collider> &newList)
{
  for (size_t i = 0; i < _bombTab.size(); i++) {
    if (_bombTab[i].isPlaced() == false) {
      _bombTab[i].placeBomb(_model.getPosition(), newList);
      break;
    }
  }
}

void Player::move(Vector2D vec)
{
  Vector3D vector = _model.getPosition();

  vector.set_x(vector.get_x() + vec.get_x());
  vector.set_z(vector.get_z() + vec.get_y());
...
void Player::movePlayer(const CommandMode control, const std::vector<Collider> &newList)
{
  ...
  _model.incrementBoxPositionX(vec.get_x());
  _model.incrementBoxPositionZ(vec.get_y());
  col.set(_model.getBoundingBox(), entityid::PLAYER);
  if (!checkCollision(Collisions::checkCollisions(newList, col))) {
    move(vec);
  } else {
    _model.incrementBoxPositionX(vec.get_x() * -1);
    _model.incrementBoxPositionZ(vec.get_y() * -1);
  }
