...
#include "../raylib_encapsulation/Audio.hpp"
#include <vector>
#include <utility>
#include <algorithm>
#include <memory>
...
Core::Core()
{
  ...
  _parameters.getTheMusic().init();
  _parameters.setMusicVolume(0.7);
...
int Core::startCore()
{
  _parameters.getTheMusic().playMusic();
  while (!_window->shouldClose() && _parameters.getCurrentScene() != QUIT)
  {
    _parameters.getTheMusic().setVolume(_parameters.getMusicVolume());
    _parameters.getTheMusic().update();
  ...
  _parameters.getTheMusic().destroy();
  Audio::closeAudio();
  _window->close();
  return (0);
}
